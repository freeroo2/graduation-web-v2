<template>
  <div id="main" class="map" ref="mapDom" />
</template>
...
onMounted(() => {
  ...
  map.on("click", function (res: any) {
    ...
    axios
      .get(
        `http://geo.datav.aliyun.com/areas_v3/bound/${childrenId.value}_full.json`
      )
      .then((res: any) => {
  ...
  axios
    .get(`http://geo.datav.aliyun.com/areas_v3/bound/100000_full.json`)
    .then((res: any) => {
      // 存一次数据 用来找子级
  ...
  axios
    .get("/api_map/news/wap/fymap2020_data.d.json?_=1585397547299")
    .then((res: any) => {
      // 将数据备份一次 用来查找子级
